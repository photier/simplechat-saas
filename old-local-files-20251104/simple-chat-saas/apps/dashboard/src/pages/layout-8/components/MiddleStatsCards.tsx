...
export const MiddleStatsCards = ({ data, loading }: MiddleStatsCardsProps) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4" style={{ gap: '25px' }}>
      {/* Total Sessions */}
      <div
        className="bg-white rounded-xl p-6 transition-all duration-300 hover:-translate-y-1 border border-gray-100"
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">{data?.onlineNow || 0} {t('common:common.online')}</p>
          </div>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">{t('stats.uniqueVisitors')}</p>
          </div>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">{t('stats.todayUsers')}</p>
          </div>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-gray-600">{t('stats.totalMessages')}</p>
            <p className="text-3xl font-bold text-gray-900 mt-1">
              {loading ? '...' : data?.totalMessages?.toLocaleString('tr-TR') || 0}
            </p>
            <p className="text-xs text-gray-500 mt-1">{t('stats.allConversations')}</p>
          </div>
        </div>
      </div>
    </div>
