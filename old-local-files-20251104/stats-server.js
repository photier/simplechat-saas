...
// Connect to widget servers to listen for real-time events
function connectToWidgetServers() {
  ...
  widgetServers.forEach(({ name, url, namespace }) => {
    ...
    socket.on('user_online', (data) => {
      console.log(`👤 [${name}] User online:`, data);
      broadcastToClients('stats_update', { event: 'user_online', source: name, ...data });
    });

    socket.on('user_offline', (data) => {
      console.log(`👋 [${name}] User offline:`, data);
      broadcastToClients('stats_update', { event: 'user_offline', source: name, ...data });
    });

    socket.on('stats_update', (data) => {
      console.log(`📊 [${name}] Stats update:`, data);
      broadcastToClients('stats_update', { event: 'stats_update', source: name, ...data });
    });
...
server.listen(PORT, () => {
  ...
  // Connect to widget servers after server is running
