...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    // Heatmap: 7 days x 24 hours (all user messages from both web and premium)
    const heatmapData = Array(7).fill(0).map(() => Array(24).fill(0));
    items.forEach(item => {
      // Count all user messages (both web and premium)
      if (item.from === 'user') {
        // Convert UTC to Turkey time (UTC+3)
        const msgDate = new Date(item.createdAt);
        const turkeyTime = new Date(msgDate.getTime() + (3 * 60 * 60 * 1000));
...
function connectToWidgetServers() {
  ...
  webClient.onAny((eventName, ...args) => {
    console.log(`🔍 [intergram] Received event: ${eventName}`, args.map(a => JSON.stringify(a).substring(0, 100)));
  });

  webClient.on('stats_update', (data) => {
    console.log('📊 [intergram] Received stats_update:', JSON.stringify(data).substring(0, 200));
    console.log('🔎 [intergram] Type:', data.type, 'HasData:', !!data.data, 'UserId:', data.data?.userId);

    // Track online/offline status
    if (data.type === 'user_online' && data.data?.userId) {
      onlineUsers.add(data.data.userId);
      console.log(`👤 [intergram] User online: ${data.data.userId} (total: ${onlineUsers.size})`);
    } else if (data.type === 'user_offline' && data.data?.userId) {
      onlineUsers.delete(data.data.userId);
      console.log(`👤 [intergram] User offline: ${data.data.userId} (total: ${onlineUsers.size})`);
    }
  ...
  // Debug: Listen to ALL events
  premiumClient.onAny((eventName, ...args) => {
    console.log(`🔍 [intergram-premium] Received event: ${eventName}`, args.map(a => JSON.stringify(a).substring(0, 100)));
  });
  ...
  premiumClient.on('stats_update', (data) => {
    ...
    if (data.type === 'user_online' && data.data?.userId) {
      onlineUsers.add(data.data.userId);
      console.log(`👤 [intergram-premium] User online: ${data.data.userId} (total: ${onlineUsers.size})`);
    } else if (data.type === 'user_offline' && data.data?.userId) {
      onlineUsers.delete(data.data.userId);
      console.log(`👤 [intergram-premium] User offline: ${data.data.userId} (total: ${onlineUsers.size})`);
    }
