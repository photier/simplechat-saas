...
function broadcastStatsUpdate(type, data) {
	...
	console.log('📡 Broadcasting to stats:', type, data.userId || '', 'to', statsIO.sockets.size, 'clients');
...
app.post('/send-to-user', function (req, res) {
		...
		const prefixedUserId = userId.startsWith('W-') || userId.startsWith('P-') ? userId : 'W-' + userId;
...
io.on('connection', function (client) {
        ...
        // Extract IP address from socket (MOVED OUTSIDE register event, like premium)
        const clientIp = client.handshake.headers['cf-connecting-ip'] ||  // Cloudflare header first!
        ...
        console.log('Client connected from IP:', clientIp, 'Country:', country, 'City:', city);
       ...
       console.log('🔵 REGISTER EVENT TRIGGERED!', registerMsg);
                ...
                try {
                        ...
                        const prefixedUserId = userId.startsWith(USER_ID_PREFIX) ? userId : USER_ID_PREFIX + userId;
                        ...
                        request.post({
                                ...
                                if (err) {
                                        ...
                                        console.log('📡 Broadcasting widget_opened to', statsIO.sockets.size, 'clients');
                                        for (const [id, socket] of statsIO.sockets) {
                                                socket.emit('widget_opened', widgetOpenData);
                                        }
                                }
                        });
                } catch (error) {
                ...
                // Send service message if enabled
