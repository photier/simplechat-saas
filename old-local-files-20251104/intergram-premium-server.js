...
io.on('connection', function (client) {
	...
	client.on('register', function (registerMsg) {
		...
		const prefixedUserId = userId.startsWith('P-') ? userId : 'P-' + userId; // Use prefix for N8N/Telegram/Stats
		...
		client.on('message', function (msg) {
			...
			const prefixedUserId = userId.startsWith('P-') ? userId : 'P-' + userId;
...
app.post('/send-to-user', function (req, res) {
	try {
		...
		// Keep userId as-is (with prefix)
		const cleanUserId = userId;

		console.log('n8n response > ' + message, 'to userId:', cleanUserId, 'from:', from, 'human_mode:', human_mode);
