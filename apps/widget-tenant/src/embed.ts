...
async function fetchServerSettings(host: string): Promise<Partial<ChatConfiguration>> {
  try {
    ...
    console.warn('[SimpleChat] Failed to fetch server settings, using defaults:', error);
    return {};
  }
}
...
async function initSimpleChat() {
  ...
  if (!config) {
    console.error('[SimpleChat] Missing configuration. Please set window.simpleChatConfig');
    return;
  }

  if (!config.chatId) {
    console.error('[SimpleChat] Missing required field: chatId');
    return;
  }

  if (!config.userId) {
    console.error('[SimpleChat] Missing required field: userId');
    return;
  }

  if (!config.host) {
    console.error('[SimpleChat] Missing required field: host');
    return;
  }

  console.log('[SimpleChat] Initializing widget...');
