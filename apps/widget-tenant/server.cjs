...
// Helper function to ensure userId has the correct prefix (without duplicating)
function ensureUserIdPrefix(userId) {
  // If userId already has a prefix (W- or P-), don't add another one
...
io.on('connection', function (client) {
        ...
        client.on('register', function (registerMsg) {
                ...
                try {
                        ...
                        // Ensure userId has correct prefix (don't duplicate if already has one)
                        const prefixedUserId = ensureUserIdPrefix(userId);
...
app.get('/', function (req, res) {
	...
	if (subdomain && subdomain.startsWith('bot_')) {
		...
		// Serve the widget HTML (same as Photier's approach)
		res.sendFile(__dirname + '/static/index.html');
	} else {
		// Unknown subdomain or direct access - redirect to main site
