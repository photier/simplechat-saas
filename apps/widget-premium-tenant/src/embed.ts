...
async function initSimpleChat() {
  ...
  styleElement.textContent = `
    ...
    @import url('${host}/css/simple-chat-premium.css');
  `;
  shadowRoot.appendChild(styleElement);

  // Fetch and inject CSS content directly (more reliable than @import)
  fetch(`${host}/css/simple-chat-premium.css`)
    .then((response) => response.text())
    .then((css) => {
      const style = document.createElement('style');
      style.textContent = css;
      shadowRoot.appendChild(style);
      console.log('[SimpleChat Premium] ✓ CSS injected into Shadow DOM');
    })
    .catch((error) => {
      console.warn('[SimpleChat Premium] Failed to load CSS:', error);
    });
  ...
  console.log('[SimpleChat Premium] ✓ Widget initialized successfully with Shadow DOM');
