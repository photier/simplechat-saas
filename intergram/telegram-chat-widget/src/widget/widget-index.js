...
function injectChat() {
    if (!window.intergramId) {
        ...
        .then(result => {
            console.log('Usage start result:', result);
            if (result.online) {
                render(
                ...
                console.warn('Widget not rendered: chat is offline');
            }
        })
        .catch(error => {
            console.error('Error fetching usage-start:', error);
            // Render widget anyway on error (fail gracefully)
            render(
                <Widget intergramId={chatId}
                    CustomData={CustomData}
                    host={host}
